...
case51:
	...
	jmp	done
      ...
      movq  %rsp,%rsi
      leaq  1(%rsp),%rdx*/
      #leaq	-1(%rsp), %rsi            #save place for 4 byte to len1
	#movq	%rsp, %rsi            #turn place to rsi before the scanf 

      
	#leaq	-28(%rbp), %rdx		#allocate place to char
	#leaq	-32(%rbp), %rsi		#allocate place to char
	#movq	%rax, %rsi
	/*movl	$.scanf1, %edi
	movl	$0, %eax		#initalize before get into function
	call	scanf*/

      /*movzbl	(%rsp), %r15d   	#the new char
	movsbl %r15b, %edx		#the new char-move to parameter 3 to the function replaceChar
	movzbl	 1(%rsp), %r14d	#the old char
	#movsbl	%al, %ecx	#the old char
	movsbl	%r14b, %esi		#the old char-move to parameter 2 to the function replaceChar
      leaq    2(%rsp), %rsp
	...
	leaq	1(%rax), %r8  	#move pointer from beginning p2 to p2->str

	#movzbl	(%rsp), %r15d   	#the new char
	#movsbl %r15b, %edx		#the new char-move to parameter 3 to the function replaceChar
	#movzbl	 1(%rsp), %r14d	#the old char
	#movsbl	%al, %ecx	#the old char
	movsbl	%r14b, %esi		#the new char-move to parameter 2 to the function replaceChar
	movq	%r13, %rdi	#the pstring-p1
	#movl	%ecx, %esi
	...
	#movzbl	 %r13d, %eax	   #the new char
	#movsbl	%al, %edx		#move string to parameter third
	movsbl	%r15b, %edx		#move string to parameter third

	#movzbl	%r14b, %eax		#the old char
	#movsbl	%al, %eax
	movsbl	%r14b, %esi		#move string to parameter second
